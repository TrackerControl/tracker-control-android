...
void block_udp(const struct arguments *args,
    ...
    // Send ICMP Port Unreachable to immediately notify the sender,
    // preventing long timeouts that waste battery and block app UI
    if (version == 4) {
        // Build ICMP Destination Unreachable / Port Unreachable response
        // Include original IP header + first 8 bytes of UDP payload per RFC 792
        size_t orig_len = sizeof(struct iphdr) + 8; // IP header + 8 bytes of original datagram
        if (length < orig_len)
            orig_len = length;
        size_t icmp_len = sizeof(struct iphdr) + 8 + orig_len; // IP + ICMP header(8) + original data
        uint8_t *buffer = ng_malloc(icmp_len, "icmp4 udp block");
        memset(buffer, 0, icmp_len);

        // Build IP header (swap src/dst)
        struct iphdr *rip4 = (struct iphdr *) buffer;
        rip4->version = 4;
        rip4->ihl = sizeof(struct iphdr) >> 2;
        rip4->tot_len = htons(icmp_len);
        rip4->ttl = IPDEFTTL;
        rip4->protocol = IPPROTO_ICMP;
        rip4->saddr = ip4->daddr; // from destination
        rip4->daddr = ip4->saddr; // to source
        rip4->check = ~calc_checksum(0, (uint8_t *) rip4, sizeof(struct iphdr));

        // Build ICMP header
        uint8_t *icmp_data = buffer + sizeof(struct iphdr);
        icmp_data[0] = ICMP_UNREACH;        // type
        icmp_data[1] = ICMP_UNREACH_PORT;   // code
        // bytes 2-3: checksum (filled below)
        // bytes 4-7: unused (already zero)
        // Copy original IP header + 8 bytes of original datagram
        memcpy(icmp_data + 8, pkt, orig_len);
        // Calculate ICMP checksum
        uint16_t check = calc_checksum(0, icmp_data, 8 + orig_len);
        *(uint16_t *)(icmp_data + 2) = ~check;

        ssize_t res = write(args->tun, buffer, icmp_len);
        if (res < 0)
            log_android(ANDROID_LOG_WARN, "ICMP write error %d: %s", errno, strerror(errno));
        else
            log_android(ANDROID_LOG_INFO, "Sent ICMP port unreachable for blocked UDP %s/%u",
                        dest, ntohs(udphdr->dest));

        if (pcap_file != NULL)
            write_pcap_rec(buffer, (size_t) (res >= 0 ? res : icmp_len));

        ng_free(buffer, __FILE__, __LINE__);
    } else {
        // ICMPv6 Destination Unreachable / Port Unreachable
        size_t orig_len = length;
        if (orig_len > 1232) // Ensure packet fits in minimum IPv6 MTU
            orig_len = 1232;
        size_t icmp6_len = sizeof(struct ip6_hdr) + 8 + orig_len; // IPv6 + ICMPv6 header(8) + original
        uint8_t *buffer = ng_malloc(icmp6_len, "icmp6 udp block");
        memset(buffer, 0, icmp6_len);

        // Build IPv6 header (swap src/dst)
        struct ip6_hdr *rip6 = (struct ip6_hdr *) buffer;
        rip6->ip6_ctlun.ip6_un1.ip6_un1_flow = 0;
        rip6->ip6_ctlun.ip6_un2_vfc = IPV6_VERSION;
        rip6->ip6_ctlun.ip6_un1.ip6_un1_plen = htons(8 + orig_len);
        rip6->ip6_ctlun.ip6_un1.ip6_un1_nxt = IPPROTO_ICMPV6;
        rip6->ip6_ctlun.ip6_un1.ip6_un1_hlim = IPDEFTTL;
        memcpy(&rip6->ip6_src, &ip6->ip6_dst, 16); // from destination
        memcpy(&rip6->ip6_dst, &ip6->ip6_src, 16); // to source

        // Build ICMPv6 header
        uint8_t *icmp_data = buffer + sizeof(struct ip6_hdr);
        icmp_data[0] = ICMP6_DST_UNREACH;      // type
        icmp_data[1] = ICMP6_DST_UNREACH_NOPORT; // code
        // bytes 2-3: checksum (filled below)
        // bytes 4-7: unused (already zero)
        memcpy(icmp_data + 8, pkt, orig_len);
        ...
        check = calc_checksum(check, (uint8_t *) &rip6->ip6_src, 16);
        check = calc_checksum(check, (uint8_t *) &rip6->ip6_dst, 16);
        uint32_t plen = htonl(8 + orig_len);
        check = calc_checksum(check, (uint8_t *) &plen, 4);
        uint32_t nxt = htonl(IPPROTO_ICMPV6);
        check = calc_checksum(check, (uint8_t *) &nxt, 4);
        check = calc_checksum(check, icmp_data, 8 + orig_len);
        *(uint16_t *)(icmp_data + 2) = ~check;

        ssize_t res = write(args->tun, buffer, icmp6_len);
        if (res < 0)
            log_android(ANDROID_LOG_WARN, "ICMPv6 write error %d: %s", errno, strerror(errno));
        else
            log_android(ANDROID_LOG_INFO, "Sent ICMPv6 port unreachable for blocked UDP %s/%u",
                        dest, ntohs(udphdr->dest));

        if (pcap_file != NULL)
            write_pcap_rec(buffer, (size_t) (res >= 0 ? res : icmp6_len));
