...
uint16_t get_mtu() { return 10000; }

uint16_t get_default_mss(int version) {
  if (version == 4)
    return (uint16_t)(get_mtu() - sizeof(struct iphdr) - sizeof(struct tcphdr));
  else
    return (uint16_t)(get_mtu() - sizeof(struct ip6_hdr) -
                      sizeof(struct tcphdr));
}

int check_tun(const struct arguments *args, const struct epoll_event *ev,
              const int epoll_fd, int sessions, int maxsessions) {
  // Check tun error
  if (ev->events & EPOLLERR) {
    ...
    if (fcntl(args->tun, F_GETFL) < 0) {
      ...
      report_exit(args, "fcntl tun %d F_GETFL error %d: %s", args->tun, errno,
                  strerror(errno));
    } else
  ...
  if (ev->events & EPOLLIN) {
    ...
    if (length < 0) {
      ...
      log_android(ANDROID_LOG_ERROR, "tun %d read error %d: %s", args->tun,
                  errno, strerror(errno));
      if (errno == EINTR || errno == EAGAIN)
        // Retry later
        return 0;
      else {
        report_exit(args, "tun %d read error %d: %s", args->tun, errno,
                    strerror(errno));
        return -1;
      }
    } else if (length > 0) {
...
void handle_ip(const struct arguments *args, const uint8_t *pkt,
               const size_t length, const int epoll_fd, int sessions,
               int maxsessions) {
  uint8_t protocol;
  void *saddr;
  void *daddr;
  ...
  *data = 0;
  if (protocol == IPPROTO_ICMP || protocol == IPPROTO_ICMPV6) {
    ...
    // TODO checksum
  } else if (protocol != IPPROTO_HOPOPTS && protocol != IPPROTO_IGMP &&
  ...
  log_android(ANDROID_LOG_DEBUG,
              "Packet v%d %s/%u > %s/%u proto %d flags %s uid %d", version,
              source, sport, dest, dport, protocol, flags, uid);

  // Check if allowed
  //
  // SNI interception for port 443:
  //   SYN         → pass to handle_tcp (creates session with socket=-1, fake
  //   SYN-ACK) ACK (no data) → complete fake handshake, await Client Hello Data
  //   (Client Hello) → parse SNI, allow/block, open real socket if allowed
  //   Subsequent   → forward via handle_tcp once real socket exists
  int allowed = 0;
  struct allowed *redirect = NULL;
  if (protocol == IPPROTO_UDP && has_udp_session(args, pkt, payload))
    allowed = 1; // could be a lingering/blocked session
  else if (protocol == IPPROTO_TCP && dport == 443 && syn)
    allowed = 1; // pass SYN to handle_tcp for fake SYN-ACK
  else if (protocol == IPPROTO_TCP && dport == 443 && !syn) {
    // Port 443 non-SYN: SNI interception state machine
    const uint8_t tcpversion = (*pkt) >> 4;
    const struct iphdr *ip4 = (struct iphdr *)pkt;
    const struct ip6_hdr *ip6 = (struct ip6_hdr *)pkt;
    const struct tcphdr *tcphdr = (struct tcphdr *)payload;
    const uint8_t tcpoptlen = (uint8_t)((tcphdr->doff - 5) * 4);
    const uint8_t *tcpdata = payload + sizeof(struct tcphdr) + tcpoptlen;
    const uint16_t datalen = (const uint16_t)(length - (tcpdata - pkt));

    // Search existing TCP session
    struct ng_session *cur = args->ctx->ng_session;
    while (cur != NULL &&
           !(cur->protocol == IPPROTO_TCP && cur->tcp.version == tcpversion &&
             cur->tcp.source == tcphdr->source &&
             cur->tcp.dest == tcphdr->dest &&
             (tcpversion == 4
    ...
    // State 1: Awaiting Client Hello (socket=-1, hostname not checked, has
    // data)
    if (cur != NULL && cur->socket == -1 && cur->tcp.checkedHostname == 0 &&
        datalen > 0) {
      // Try to parse Server Name Extension
      char hostname[512] = "";
      parse_tls_header((const char *)tcpdata, datalen, hostname);
      ...
      // Check if this address (with SNI hostname) is allowed
      jobject objPacket = create_packet(args, version, protocol, flags, source,
                                        sport, dest, dport, packetdata, uid, 0);
      ...
      if (allowed) {
        ...
        if (cur->socket < 0) {
          log_android(ANDROID_LOG_ERROR,
                      "SNI intercept: failed to open socket for %s", dest);
          write_rst(args, &cur->tcp);
        } else {
          log_android(ANDROID_LOG_INFO,
                      "SNI intercept: opened socket for allowed %s (SNI: %s)",
                      dest, packetdata);

          // Monitor events on the real socket
          cur->ev.events = EPOLLOUT | EPOLLERR;
          cur->ev.data.ptr = cur;
          if (epoll_ctl(epoll_fd, EPOLL_CTL_ADD, cur->socket, &cur->ev))
            log_android(ANDROID_LOG_ERROR, "epoll add tcp sni error %d: %s",
                        errno, strerror(errno));

          cur->tcp.recv_window = get_receive_window(cur);

          // Queue the Client Hello for forwarding after socket connects
          // Don't ACK — the forward loop will ACK after sending
          if (datalen > 0)
            queue_tcp(args, tcphdr, "SNI-intercept", &cur->tcp, tcpdata,
                      datalen);
        }
      } else {
        log_android(ANDROID_LOG_WARN, "SNI intercept: blocked %s (SNI: %s)",
                    dest, packetdata);
        write_rst(args, &cur->tcp);
      }
    } else if (cur != NULL && cur->socket == -1 &&
               cur->tcp.checkedHostname == 0 && datalen == 0) {
      ...
      handle_tcp(args, pkt, length, payload, uid, allowed, redirect, epoll_fd);
               ...
               cur->tcp.checkedHostname == 1) {
      // State 4: Blocked session teardown — ACK remaining data
      cur->tcp.remote_seq = ntohl(tcphdr->seq) + datalen;
      write_ack(args, &cur->tcp);
    } else {
      // No matching session — pass to handle_tcp
      allowed = 1;
      handle_tcp(args, pkt, length, payload, uid, allowed, redirect, epoll_fd);
    }
  } else if (protocol == IPPROTO_TCP &&
             ((!syn && dport != 443)         // assume existing session
              || (uid == 0 && dport == 53))) // assume existing session
    allowed = 1;
  else {
    // Normal allow/block check for non-443 TCP, ICMP, UDP
    char *packetdata = data;
  ...
  // Handle allowed traffic (skip for port 443 non-SYN which is handled above)
  if (protocol == IPPROTO_TCP && dport == 443 && !syn) {
    // Port 443 data traffic is fully handled in the SNI interception logic
    // above
  } else if (allowed) {
    if (protocol == IPPROTO_ICMP || protocol == IPPROTO_ICMPV6)
...
jint get_uid(const int version, const int protocol, const void *saddr,
             const uint16_t sport, const void *daddr, const uint16_t dport) {
  jint uid = -1;
  ...
  if (version == 4) {
    ...
    uid = get_uid_sub(6, protocol, saddr128, sport, daddr128, dport, source,
                      dest, now);
  ...
  if (uid == -1) {
    uid = get_uid_sub(version, protocol, saddr, sport, daddr, dport, source,
                      dest, now);
  ...
  if (uid == -1)
    ...
    log_android(ANDROID_LOG_INFO, "uid v%d p%d %s/%u > %s/%u => %d", version,
                protocol, source, sport, dest, dport, uid);
...
jint get_uid_sub(const int version, const int protocol, const void *saddr,
                 const uint16_t sport, const void *daddr, const uint16_t dport,
  ...
  FILE *fd = fopen(fn, "r");
  if (fd == NULL) {
    log_android(ANDROID_LOG_ERROR, "fopen %s error %d: %s", fn, errno,
                strerror(errno));
    return -2;
  }
  ...
  *line = 0;
  ...
  while (fgets(line, sizeof(line), fd) != NULL) {
    ...
    if (fields == 5 && strlen(shex) == ws * 8 && strlen(dhex) == ws * 8) {
      ...
      if (_sport == sport && (_dport == dport || _dport == 0) &&
  ...
  if (fclose(fd))
    log_android(ANDROID_LOG_ERROR, "fclose %s error %d: %s", fn, errno,
                strerror(errno));
