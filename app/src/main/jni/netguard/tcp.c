...
int check_tcp_session(const struct arguments *args, struct ng_session *s,
  ...
  char session[250];
  sprintf(session, "TCP socket from %s/%u to %s/%u %s socket %d", source,
  ...
  if (s->tcp.state != TCP_CLOSING && s->tcp.state != TCP_CLOSE &&
    ...
    log_android(ANDROID_LOG_WARN, "%s idle %d/%d sec ", session,
                now - s->tcp.time, timeout);
  ...
  if (s->tcp.state == TCP_CLOSING) {
    ...
    if (s->socket >= 0) {
      if (close(s->socket))
        log_android(ANDROID_LOG_ERROR, "%s close error %d: %s", session, errno,
                    strerror(errno));
      else
  ...
  if ((s->tcp.state == TCP_CLOSING || s->tcp.state == TCP_CLOSE) &&
    ...
    account_usage(args, s->tcp.version, IPPROTO_TCP, dest, ntohs(s->tcp.dest),
                  s->tcp.uid, s->tcp.sent, s->tcp.received);
    s->tcp.sent = 0;
    s->tcp.received = 0;
  }
...
int monitor_tcp_session(const struct arguments *args, struct ng_session *s,
  ...
  if (events != s->ev.events) {
    ...
    if (epoll_ctl(epoll_fd, EPOLL_CTL_MOD, s->socket, &s->ev)) {
      s->tcp.state = TCP_CLOSING;
      log_android(ANDROID_LOG_ERROR, "epoll mod tcp error %d: %s", errno,
                  strerror(errno));
    } else
...
uint32_t get_receive_buffer(const struct ng_session *cur) {
  ...
  if (getsockopt(cur->socket, SOL_SOCKET, SO_SNDBUF, &sendbuf,
                 (socklen_t *)&sendbufsize) < 0)
    log_android(ANDROID_LOG_WARN, "getsockopt SO_RCVBUF %d: %s", errno,
                strerror(errno));
  ...
  if (ioctl(cur->socket, SIOCOUTQ, &unsent))
    log_android(ANDROID_LOG_WARN, "ioctl SIOCOUTQ %d: %s", errno,
                strerror(errno));

  uint32_t total = (uint32_t)(unsent < sendbuf ? sendbuf - unsent : 0);

  log_android(ANDROID_LOG_DEBUG, "Send buffer %u unsent %u total %u", sendbuf,
              unsent, total);
...
void check_tcp_socket(const struct arguments *args,
                      const struct epoll_event *ev, const int epoll_fd) {
  ...
  char session[250];
  sprintf(session, "TCP socket from %s/%u to %s/%u %s loc %u rem %u", source,
          ntohs(s->tcp.source), dest, ntohs(s->tcp.dest),
          strstate(s->tcp.state), s->tcp.local_seq - s->tcp.local_start,
  ...
  if (ev->events & EPOLLERR) {
    ...
    if (err < 0)
      log_android(ANDROID_LOG_ERROR, "%s getsockopt error %d: %s", session,
                  errno, strerror(errno));
    else if (serr)
      log_android(ANDROID_LOG_ERROR, "%s SO_ERROR %d: %s", session, serr,
                  strerror(serr));
    ...
    if (s->tcp.state == TCP_LISTEN) {
        ...
        if (ev->events & EPOLLIN) {
          ...
          if (bytes < 0) {
            ...
            if (s->tcp.socks5 == SOCKS5_HELLO && bytes == 2 && buffer[0] == 5) {
              ...
              s->tcp.socks5 = 0;
              log_android(ANDROID_LOG_ERROR, "%s recv SOCKS5 state %d", session,
                          s->tcp.socks5);
              write_rst(args, &s->tcp);
            }
          }
      ...
      if (s->tcp.socks5 == SOCKS5_HELLO) {
        ...
        log_android(ANDROID_LOG_INFO, "%s sending SOCKS5 hello: %s", session,
                    h);
        ng_free(h, __FILE__, __LINE__);
        ...
        *(buffer + 2 + ulen) = plen;
        ...
        log_android(ANDROID_LOG_INFO, "%s sending SOCKS5 auth: %s", session, h);
        ng_free(h, __FILE__, __LINE__);
        ssize_t sent = send(s->socket, buffer, len, MSG_NOSIGNAL);
        if (sent < 0) {
          log_android(ANDROID_LOG_ERROR, "%s send SOCKS5 connect error %d: %s",
                      session, errno, strerror(errno));
          write_rst(args, &s->tcp);
        }
        ...
        *(buffer + 3) = (uint8_t)(s->tcp.version == 4 ? 1 : 4);
        ...
        char *h = hex(buffer, len);
        log_android(ANDROID_LOG_INFO, "%s sending SOCKS5 connect: %s", session,
                    h);
        ng_free(h, __FILE__, __LINE__);
        ssize_t sent = send(s->socket, buffer, len, MSG_NOSIGNAL);
        if (sent < 0) {
      ...
      if (ev->events & EPOLLOUT) {
        ...
        while (s->tcp.forward != NULL &&
               ...
               s->tcp.forward->len - s->tcp.forward->sent < buffer_size) {
          log_android(ANDROID_LOG_DEBUG, "%s fwd %u...%u sent %u", session,
                      s->tcp.forward->seq - s->tcp.remote_start,
                      s->tcp.forward->seq + s->tcp.forward->len -
                          s->tcp.remote_start,
                      s->tcp.forward->sent);

          ssize_t sent =
              send(s->socket, s->tcp.forward->data + s->tcp.forward->sent,
                   s->tcp.forward->len - s->tcp.forward->sent,
                   (unsigned int)(MSG_NOSIGNAL |
                                  (s->tcp.forward->psh ? 0 : MSG_MORE)));
          if (sent < 0) {
            log_android(ANDROID_LOG_ERROR, "%s send error %d: %s", session,
                        errno, strerror(errno));
        ...
        while (seg != NULL) {
          log_android(ANDROID_LOG_WARN, "%s queued %u...%u sent %u", session,
                      seg->seq - s->tcp.remote_start,
                      seg->seq + seg->len - s->tcp.remote_start, seg->sent);
          seg = seg->next;
        }
      }
      ...
      if ((prev == 0 && window > 0) || (prev > 0 && window == 0))
        log_android(ANDROID_LOG_WARN, "%s recv window %u > %u", session, prev,
                    window);
      ...
      if (s->tcp.state == TCP_ESTABLISHED || s->tcp.state == TCP_CLOSE_WAIT) {
        ...
        if ((ev->events & EPOLLIN) && send_window > 0) {
          ...
          if (bytes < 0) {
            ...
            if (close(s->socket))
              log_android(ANDROID_LOG_ERROR, "%s close error %d: %s", session,
                          errno, strerror(errno));
            s->socket = -1;
...
jboolean handle_tcp(const struct arguments *args, const uint8_t *pkt,
  ...
  char packet[250];
  sprintf(packet, "TCP %s %s/%u > %s/%u seq %u ack %u data %u win %u uid %d",
          flags, source, ntohs(tcphdr->source), dest, ntohs(tcphdr->dest),
          ntohl(tcphdr->seq) - (cur == NULL ? 0 : cur->tcp.remote_start),
          tcphdr->ack ? ntohl(tcphdr->ack_seq) -
                            (cur == NULL ? 0 : cur->tcp.local_start)
                      : 0,
  ...
  if (cur == NULL) {
    if (tcphdr->syn) {
      ...
      // Register session
      struct ng_session *s =
          ng_malloc(sizeof(struct ng_session), "tcp session");
      ...
      s->tcp.recv_scale = ws;
      s->tcp.send_scale = ws;
      s->tcp.send_window = ((uint32_t)ntohs(tcphdr->window))
                           << s->tcp.send_scale;
      s->tcp.unconfirmed = 0;
      ...
      // Port 443: send fake SYN-ACK without connecting to the remote server.
      // ip.c will parse SNI from the Client Hello and decide allow/block.
      uint16_t dport = ntohs(tcphdr->dest);
      if (dport == 443) {
        s->socket = -1;
        s->tcp.checkedHostname = 0;
        s->tcp.recv_window = 65535;
        ...
        log_android(ANDROID_LOG_WARN,
                    "%s SNI intercept: fake SYN-ACK, no remote socket", packet);
      } else {
        // Open socket
        ...
        log_android(ANDROID_LOG_DEBUG, "TCP socket %d lport %d", s->socket,
                    get_local_port(s->socket));
      ...
      rst.remote_seq =
          ntohl(tcphdr->seq) + datalen + (tcphdr->syn || tcphdr->fin ? 1 : 0);
    ...
    char session[250];
    sprintf(session, "%s %s loc %u rem %u acked %u", packet,
            strstate(cur->tcp.state), cur->tcp.local_seq - cur->tcp.local_start,
    ...
    if (cur->tcp.state == TCP_CLOSING || cur->tcp.state == TCP_CLOSE) {
      ...
      cur->tcp.send_window = ((uint32_t)ntohs(tcphdr->window))
                             << cur->tcp.send_scale;
      ...
      if (datalen) {
        ...
        if (cur->tcp.state == TCP_CLOSE_WAIT) {
          log_android(ANDROID_LOG_ERROR, "%s data while remote closed",
                      session);
          write_rst(args, &cur->tcp);
          return 0;
        }
          ...
          if ((uint32_t)(ack + 1) == cur->tcp.local_seq) {
            ...
            if (cur->tcp.state == TCP_ESTABLISHED) {
              ...
              if (setsockopt(cur->socket, SOL_SOCKET, SO_KEEPALIVE, &on,
                ...
                log_android(ANDROID_LOG_ERROR,
                            "%s setsockopt SO_KEEPALIVE error %d: %s", session,
                            errno, strerror(errno));
              else
            ...
            if (compare_u32(ack, cur->tcp.acked) <= 0)
              log_android(
                  ...
                  "%s repeated ACK %u/%u", session, ack - cur->tcp.local_start,
                  cur->tcp.acked - cur->tcp.local_start);
            else {
              log_android(ANDROID_LOG_WARN, "%s previous ACK %u", session,
                          ack - cur->tcp.local_seq);
              cur->tcp.acked = ack;
            }
      ...
      if (cur->tcp.state != oldstate || cur->tcp.local_seq != oldlocal ||
          cur->tcp.remote_seq != oldremote)
        log_android(ANDROID_LOG_INFO, "%s > %s loc %u rem %u", session,
...
void queue_tcp(const struct arguments *args, const struct tcphdr *tcphdr,
               const char *session, struct tcp_session *cur,
               const uint8_t *data, uint16_t datalen) {
  uint32_t seq = ntohl(tcphdr->seq);
  if (compare_u32(seq, cur->remote_seq) < 0)
    log_android(ANDROID_LOG_WARN, "%s already forwarded %u..%u", session,
    ...
    if (s == NULL || compare_u32(s->seq, seq) > 0) {
      log_android(ANDROID_LOG_DEBUG, "%s queuing %u...%u", session,
                  seq - cur->remote_start, seq + datalen - cur->remote_start);
      ...
      if (s->len == datalen)
        log_android(ANDROID_LOG_WARN, "%s segment already queued %u..%u",
                    session, s->seq - cur->remote_start,
                    s->seq + s->len - cur->remote_start);
      else if (s->len < datalen) {
        log_android(ANDROID_LOG_WARN, "%s segment smaller %u..%u > %u", session,
        ...
        log_android(ANDROID_LOG_ERROR, "%s segment larger %u..%u < %u", session,
                    s->seq - cur->remote_start,
                    s->seq + s->len - cur->remote_start,
                    s->seq + datalen - cur->remote_start);
        ng_free(s->data, __FILE__, __LINE__);
...
int open_tcp_socket(const struct arguments *args, const struct tcp_session *cur,
  ...
  if ((sock = socket(version == 4 ? PF_INET : PF_INET6, SOCK_STREAM, 0)) < 0) {
    log_android(ANDROID_LOG_ERROR, "socket error %d: %s", errno,
                strerror(errno));
    return -1;
  }
  ...
  if (setsockopt(sock, SOL_TCP, TCP_NODELAY, &on, sizeof(on)) < 0)
    log_android(ANDROID_LOG_ERROR, "setsockopt TCP_NODELAY error %d: %s", errno,
                strerror(errno));
  ...
  if (redirect == NULL) {
    if (*socks5_addr && socks5_port) {
      log_android(ANDROID_LOG_WARN, "TCP%d SOCKS5 to %s/%u", version,
    ...
    log_android(ANDROID_LOG_WARN, "TCP%d redirect to %s/%u", version,
                redirect->raddr, redirect->rport);
  ...
  int err = connect(sock,
                    ...
                    (socklen_t)(version == 4 ? sizeof(struct sockaddr_in)
                                             : sizeof(struct sockaddr_in6)));
  if (err < 0 && errno != EINPROGRESS) {
    log_android(ANDROID_LOG_ERROR, "connect error %d: %s", errno,
                strerror(errno));
    return -1;
  }
...
ssize_t write_tcp(const struct arguments *args, const struct tcp_session *cur,
  ...
  if (cur->version == 4) {
    ...
    if (datalen)
      memcpy(buffer + sizeof(struct iphdr) + sizeof(struct tcphdr) + optlen,
             data, datalen);
    ...
    if (datalen)
      memcpy(buffer + sizeof(struct ip6_hdr) + sizeof(struct tcphdr) + optlen,
             data, datalen);
  ...
  log_android(ANDROID_LOG_DEBUG,
              ...
              (tcp->fin ? " FIN" : ""), (tcp->rst ? " RST" : ""), dest,
              ntohs(tcp->dest), ntohl(tcp->seq) - cur->local_start,
              ntohl(tcp->ack_seq) - cur->remote_start, datalen);
  ...
  if (res >= 0) {
    ...
    log_android(ANDROID_LOG_ERROR, "TCP write%s%s%s%s data %d error %d: %s",
                (tcp->syn ? " SYN" : ""), (tcp->ack ? " ACK" : ""),
                (tcp->fin ? " FIN" : ""), (tcp->rst ? " RST" : ""), datalen,
                errno, strerror((errno)));
